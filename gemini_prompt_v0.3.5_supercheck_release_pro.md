# Gemini 3 Pro Prompt: LyricVault v0.3.5 Final Supercheck + Build + Push Gate

You are acting as a release engineering lead. Execute a final zero-drift release gate for LyricVault v0.3.5 with evidence-backed results only.
...
4. Preserve existing release versioning (`0.3.5`) unless explicitly told otherwise.

## Execution Plan
1. Run frontend gate from `gemini_prompt_v0.3.5_frontend_gate.md`.
2. Run backend gate from `gemini_prompt_v0.3.5_backend_gate.md`.
...
- `rg -n "Â|â€”|â|ð|Ã|¤|¢|¬|\\\\uFFFD" frontend/src backend electron package.json README.md CHANGELOG.md release_notes_v0.3.5.md release_publish_v0.3.5.md smoke_test_v0.3.5.md`
- `rg -n "TODO|FIXME|XXX|debugger" frontend/src backend electron`
4. Build release artifacts:
- `npm run dist`
5. Regenerate checksums for v0.3.5 artifacts and verify against committed checksum files.
6. Validate git state for release:
- Ensure intended files are committed.
- Ensure tag `v0.3.5` points to intended release commit.
- Ensure branch/tag push commands are ready.

## Push Method (Mirror Existing Release Flow)
Use this exact command sequence if GO:
- `git push origin HEAD`
- `git tag -d v0.3.5` (if already exists and needs retarget)
